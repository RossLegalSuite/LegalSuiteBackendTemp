    ...
    methods: {
        ...
        deleteClicked() {
            ...
            if ( this.callingComponent && typeof this.callingComponent.deleteRecord === 'function') {
                ...
                this.callingComponent.deleteRecord(this.callingComponent?.record?.id); // You can override default behaviour in the calling component

            } else {

                if ( this.callingComponent && this.callingComponent.record.id) {

                    axios.post('/' + this.callingComponent.table.tableName + '/delete/' + this.callingComponent.record.id)
                    
                    .then(response => {

                        if (response.data.errors) {

                            showError( 'Error', 'An error was encountered trying to delete a record.<br><br>' + response.data.errors);
